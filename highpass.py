...
for i in range(17):
    ...
    images_temp[:,:,:,i] = mpflt.smoothing.anisotropic_diffusion(images[:,:,:,i], niter=15, option = 1, kappa = 20, gamma = 0.2)
    ...
    images_trans[:,:,:,i] = mpg.morphological_gradient(gauss_highpass[:,:,:,i], structure=ball)
...
for i in range(17):
    images_gray[:,:,i] = clr.rgb2gray(images_trans[:,:,:,i])
    blobs_log = blob_log(images_gray[:,:,i], min_sigma=25, max_sigma=40, num_sigma=10, threshold=.1)
    # Compute radii in the 3rd column.
    blobs_log[:, 2] = blobs_log[:, 2] * sqrt(2)
    blobs_dog = blob_dog(images_gray[:,:,i], min_sigma=25 ,max_sigma=40, threshold=.1)
    blobs_dog[:, 2] = blobs_dog[:, 2] * sqrt(2)
    blobs_doh = blob_doh(images_gray[:,:,i], min_sigma=25, max_sigma=40, threshold=.004)
    ...
    for idx, (blobs, color, title) in enumerate(sequence):
        ...
        ax[idx].imshow(images[:,:,:,i], cmap = 'gray')
        for blob in blobs:
            y, x, r = blob
    ...
    fig.savefig(R"blobs\blobs_{}.png".format(i), facecolor = "white",bbox_inches='tight')
#ax[3].imshow(images_gray[:,:,i], cmap = 'gray')
#ax[3].set_axis_off()
